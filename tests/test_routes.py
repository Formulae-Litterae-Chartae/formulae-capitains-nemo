...
class SSLESConfig(TestConfig):
    ELASTICSEARCH_URL = "https://some.secure.server:443"
    ES_CLIENT_CERT = "SomeFile"
    ES_CLIENT_KEY = "SomeOtherFile"


class NormalESConfig(TestConfig):
    ELASTICSEARCH_URL = "https://somehost:9200"
...
class TestInit(TestCase):
    ...
    def test_non_secure_es_server(self):
        ...
        self.assertEqual([node.base_url for node in app.elasticsearch.transport.node_pool.all()][0], NormalESConfig.ELASTICSEARCH_URL.lower())

    def test_secure_es_server(self):
        """ Make sure that an ES server with no SSL security is correctly initiated"""
        app = create_app(SSLESConfig)
        self.assertEqual([node.host for node in app.elasticsearch.transport.node_pool.all()][0],
                         'some.secure.server',
                         'Host server name should be correct.')
        print([node.config for node in app.elasticsearch.transport.node_pool.all()][0])
        self.assertTrue([node.config.verify_certs for node in app.elasticsearch.transport.node_pool.all()][0], 'CA certs should be verified.')
...
class TestIndividualRoutes(Formulae_Testing):
    def test_anonymous_user(self):
        ...
        with self.client as c:
            ...
            c.get('/corpus/urn:cts:formulae:marculf.form000.lat001', follow_redirects=True)
            self.assertIn('main::multipassage.html', [x[0].name for x in self.templates])
            self.assertEqual(self.get_context_variable('objects')[0]['objectId'], 'urn:cts:formulae:marculf.form000.lat001')
            c.get('/corpus/urn:cts:formulae:marculf.form000', follow_redirects=True)
            self.assertIn('errors::unknown_collection.html', [x[0].name for x in self.templates])
            self.assertEqual(self.get_context_variable('parent'), 'urn:cts:formulae:marculf')
            ...
            with patch('requests.get', side_effect=mocked_requests_post) as mock_post:
                ...
                coll_data = self.get_context_variable('coll_dict')
                self.assertEqual(json_data['sancti'], 2)
                self.assertEqual(['urn:cts:formulae:marculf'], [x['id'] for x in coll_data['Formulae']])
                ...
                self.assertEqual(['urn:cts:formulae:raetien'], [x['id'] for x in self.get_context_variable('coll_dict')['Urkunden']['None']])
            ...
            #self.assertIn(b'Encrypt', r.get_data()[:300], 'PDF should be encrypted.')
            pdf_bytes = r.get_data()
            self.assertTrue(b'Encrypt' in pdf_bytes, '❌ PDF is not encrypted: "Encrypt" marker not found.')
    ...
    def test_authorized_project_member(self):
        ...
        with self.client as c:
            ...
            self.assertEqual(d[0]['collections']['current']['transcribed_edition'], ['Marculf II,11'])
            c.get('/texts/manifest:urn:cts:formulae:be4.86v.lat001/passage/1', follow_redirects=True)
            self.assertIn('main::multipassage.html', [x[0].name for x in self.templates])
            d = self.get_context_variable('objects')
            self.assertEqual(d[0]['lib_link'], 'https://www.e-codices.ch/en/bbb/0611/86v')
            self.assertEqual(d[0]['collections']['current']['transcribed_edition'], ['Bourges A 7'])
